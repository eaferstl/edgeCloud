...
| D-012 | **(Proposed)** Expose edgeCloud to Edge Esmeralda attendee agents via an `edgecloud` **MCP server** (+ thin Hermes skill), as a pure **client** of the existing HTTP API — no protocol/payload change. Agents can submit pure-compute jobs and (worker role) enroll their machine. | TODO (proposed by integration analysis) | 2026-06-04 | Attendee agents already integrate tools as MCP servers + bearer tokens (EdgeOS/Geo/Index Network) and are gated by the same attendee-email allowlist edgeCloud enforces — so this composes cleanly without new trust. Rejected alternatives: a bespoke agent API (agents already speak MCP); letting agent code run its full loop on workers (hermetic sandbox forbids network/fs — pure compute only). Pending owner + Coordination ratification into `00_master_prd.md`. | `agent_mcp_integration/prd.md`, `agent_mcp_integration/spec.md`, `agent_mcp_integration/research/README.md` |
| D-013 | Migrate the genesis rendezvous server from the original box (`146.190.123.91`) to an owner-operated DigitalOcean node at **`seed.pandocloud.io`** (`64.23.224.76`), **preserving the genesis identity** (same `server-key.json` → unchanged `GENESIS_SERVER_KEY`, same `EDGECLOUD_SHARED_SALT`) and **minting a new libp2p peerId** (`12D3KooWH1nt…wnX1`); address it via **`/dns4`** (domain — later corrected to dual `/ip4`+`/dns4` for workers, see **D-014**). | Eliot | 2026-06-10 | DigitalOcean can't transfer the original droplet and the network must live under the owner's control. Preserving the key means **no attendee re-registers** and job/result history carries over (replicated via OrbitDB). A *new* peerId (not a copied `peer-key.bin`) lets old+new run during a ~1-month overlap without a duplicate-identity clash. `/dns4` decouples address from host so future moves are a DNS change only — and is exactly the shape the post-demo pure-mesh roadmap's WS3 multi-seed bootstrap wants; holding the genesis key is the WS5 prerequisite. **Rejected:** fresh genesis (forces every registered user to re-register); bare-IP move (recreates IP-pinning, blocks the WS3 bridge, and Let's Encrypt needs a name). | `shared/src/constants.js`, `worker/src/config.js`, `worker/docker-compose.yml`, `server/src/public/index.html`, `infra/DEPLOYMENT.md`, `infra/provision.md` |
| D-014 | Correct D-013's **worker-facing** addressing: `GENESIS_MULTIADDRS` lists **bare `/ip4/64.23.224.76` first**, then `/dns4`; worker `httpFallback` is **`http://64.23.224.76`** (bare IP); the node's Caddy serves an extra **plain-HTTP `http://<ip>` vhost** so workers have a no-DNS registration + registry-grace endpoint. Browsers keep using the HTTPS `/dns4` domain. | Eliot | 2026-06-10 | Validation (re-pulling a worker onto the new node) caught that the hardened worker egress firewall (D-008) **blocks in-container DNS on some hosts** — e.g. Docker Desktop, where the embedded resolver's upstream is an RFC1918 address the firewall drops, giving `EAI_AGAIN` for *all* domains. Pure `/dns4` (D-013) was thus unreachable for those workers — which is exactly why the original deployment used bare IPs. Raw-IP libp2p **and** a bare-IP HTTP vhost need no DNS and were verified working (worker shows `link:"direct"`). **Trade-off:** workers are IP-pinned again, so a future host move needs the IP updated + image re-published until worker DNS is solved fleet-wide (decentralization WS2/WS3); `/dns4` is kept as the secondary anchor for hosts where DNS works and for browsers. | `shared/src/constants.js`, `worker/src/config.js`, `worker/docker-compose.yml`, `infra/deploy-rendezvous.sh` |
...
| R-002 | No TLS — plain HTTP; user secret key lives in browser localStorage | Steve | Medium | Mitigated (genesis) | **TLS now deployed** on the genesis node: `seed.pandocloud.io` is fronted by Caddy + Let's Encrypt (auto-renew), so the webform/API are HTTPS (D-013). libp2p (ws/4001-4002) stays plain — workers, not browsers, dial it. Browser crypto still uses tweetnacl (D-003); HTTPS now makes `crypto.subtle` available as a future option. Any non-TLS server (e.g. the legacy bare-IP box during overlap) retains the original risk |
...
| R-012 | Agent key custody (D-012): a hosted/Telegram-resident agent's Ed25519 key, if held by a shared/experiment-side process, lets that holder read the agent's gated results | TODO | Medium | Open | MCP server (key custody) must run inside the attendee's own trust boundary; documented in `agent_mcp_integration/spec.md`. Decision on hosted-agent custody is an open question there |
| R-013 | "Your agent runs on the swarm" framing could overclaim: the hermetic sandbox runs only pure compute, and is not a confidentiality/safety boundary for arbitrary code (`THREAT_MODEL.md`) | TODO | Low | Open | Legal to review presentation wording (AGENT-007); `agent_mcp_integration/prd.md` Known limitations states the pure-compute boundary plainly |
