...
| R-012 | Agent key custody (D-012): a hosted/Telegram-resident agent's Ed25519 key, if held by a shared/experiment-side process, lets that holder read the agent's gated results | TODO | Medium | Open | MCP server (key custody) must run inside the attendee's own trust boundary; documented in `agent_integration/spec.md`. Decision on hosted-agent custody is an open question there |
| R-013 | "Your agent runs on the swarm" framing could overclaim: the hermetic sandbox runs only pure compute, and is not a confidentiality/safety boundary for arbitrary code (`THREAT_MODEL.md`) | TODO | Low | Open | Legal to review presentation wording (AGENT-007); `agent_integration/prd.md` Known limitations states the pure-compute boundary plainly |
